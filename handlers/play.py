...
async def playlist(client, message):
    ...
    if not queue:
        await message.reply_text("**لا يتم التشغيل الان!**")
    temp = []
    for t in queue:
        temp.append(t)
...
def r_ply(type_):
    ...
    mar = InlineKeyboardMarkup(
        [
            ...
            [       
                InlineKeyboardButton("اخـفاء القــائمـه", "cls")
            ]        
        ]
    )
    return mar
...
async def hfmm(_, message):
    ...
    message.chat.id
    if status == "تفعيل" or status == "تشغيل" or status == "On":
    ...
    elif status == "تعطيل" or status == "ايقاف" or status == "Off":
        lel = await message.reply("`Processing...`")
...
async def play(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return    
    lel = await message.reply("🔄 * جاري الطلــب...**")
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("عرض القــائـمه", callback_data="menu"),
                    InlineKeyboardButton("اخـفاء القــائمـه", callback_data="cls"),
                ],[
        ...
        title = file_name
        thumb_name = "https://i.imgur.com/TkDRyzz.jpg"
        ...
        try:
            ...
            keyboard = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("تشغيل الاختيار ال『 ❶』", callback_data=f'plll 0|{query}|{user_id}'),
                    ],
                    [
                        InlineKeyboardButton("تشغيل الاختيار ال『 ❷ 』", callback_data=f'plll 2|{query}|{user_id}'),
                    ],
                    [
                        InlineKeyboardButton("تشغيل الاختيار ال『 ❸ 』", callback_data=f'plll 3|{query}|{user_id}'),
                    ],
                    [InlineKeyboardButton(text="اخـفاء القــائمـه", callback_data="cls")],
                ]
            )
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        try:
            ...
            message.reply("**لا تتوفر مكالمه صوتيه نشطه يرجي فتحها.**")
            return
        await message.reply_photo(
            photo="final.png",
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        await cb.answer("انت لست من طلب هذه الأغنية!", show_alert=True)
        return
